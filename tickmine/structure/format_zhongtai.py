...
def main():
    from tickmine.global_config import huaxin_src_path
    from tickmine.global_config import huaxin_dst_path

    recordFileName = "record.json"

    reconstruct("%s/day" % (huaxin_src_path), huaxin_dst_path, recordFileName, isNight=False)
    reconstruct("%s/night" % (huaxin_src_path), huaxin_dst_path, recordFileName, isNight=True)
